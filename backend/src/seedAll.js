...
async function main() {
    ...
    let adminUser = await prisma_1.prisma.user.findUnique({
    ...
    for (const cat of categoriesData) {
        let dbCat = await prisma_1.prisma.category.findUnique({
            where: { name: cat.name }
        });
        if (!dbCat) {
            dbCat = await prisma_1.prisma.category.create({
                data: cat
            });
    ...
    for (const prov of providersData) {
        const existingProv = await prisma_1.prisma.provider.findFirst({
            where: { name: prov.name }
        });
        if (!existingProv) {
            await prisma_1.prisma.provider.create({
                data: prov
            });
